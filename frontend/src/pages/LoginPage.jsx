    ...
    <div style={{ minHeight: '100vh', background: colors.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ width: 380, background: colors.cardBg, border: `1px solid ${colors.border}`, borderRadius: 12, padding: 32 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 600, color: colors.text }}>CutQuote</h1>
          <p style={{ fontSize: 13, color: colors.textMuted, marginTop: 4 }}>Sign in to your account</p>
        </div>
        ...
        <form onSubmit={submit}>
          {[['Email', 'email', 'email'], ['Password', 'password', 'password']].map(([l, k, type]) => (
        ...
        <p style={{ textAlign: 'center', fontSize: 13, color: colors.textMuted, marginTop: 20 }}>
          No account? <Link to="/register" style={{ color: colors.accent, textDecoration: 'none' }}>Register</Link>
        </p>
      </div>
    </div>
  );
