...
export default function CheckoutModal({ quotes, totalExVat, onClose, onSuccess }) {
  const [form, setForm] = useState({
    name: '', street: '', city: '', postal_code: '', country: 'Bulgaria', notes: '',
  });
      ...
      <div style={{ background: '#151a25', border: '1px solid #1e293b', borderRadius: 14, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid #1e293b' }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#64748b', margin: '3px 0 0' }}>{quotes.length} part{quotes.length > 1 ? 's' : ''} · confirm delivery details</p>
          </div>
        ...
        <div style={{ padding: '14px 22px', borderBottom: '1px solid #1e293b', background: '#0f1117' }}>
          {quotes.map(q => (
            <div key={q.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '3px 0', color: '#64748b' }}>
              <span style={{ color: '#94a3b8' }}>{q.original_name} <span style={{ fontSize: 11 }}>× {q.quantity}</span></span>
              <span>€{Number(q.total_price).toFixed(2)}</span>
            </div>
          ))}
        ...
        <form onSubmit={submit} style={{ padding: '18px 22px' }}>
          ...
          <div style={{ marginTop: 6, fontSize: 12, color: '#475569', marginBottom: 16, lineHeight: 1.5 }}>
          ...
          <button type="submit" disabled={submitting} style={{
            width: '100%', padding: '12px', background: submitting ? '#1e293b' : '#22d3a5',
            color: submitting ? '#475569' : '#0f1117', border: 'none', borderRadius: 8,
...
function Field({ label, value, onChange, required, multiline }) {
  const style = { width: '100%', background: '#0f1117', border: '1px solid #1e293b', borderRadius: 6, padding: '8px 11px', fontSize: 13, color: '#f8fafc', outline: 'none', fontFamily: 'inherit', resize: 'none', boxSizing: 'border-box' };
  return (
