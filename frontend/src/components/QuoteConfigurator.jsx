...
export default function QuoteConfigurator({ file, onQuoteSaved }) {
  ...
  useEffect(() => {
    ...
    setCalculating(true);
    const t = setTimeout(() => {
      api.calculateQuote({ file_id: file.id, cutting_method: form.method, material: form.material, thickness_mm: form.thickness, quantity: form.quantity })
  ...
  const saveQuote = async () => {
    setSaving(true);
    try {
      const q = await api.saveQuote({ file_id: file.id, cutting_method: form.method, material: form.material, thickness_mm: form.thickness, quantity: form.quantity });
      setSaved(true);
      ...
      <div style={{ padding: 16 }}>
        <Section label="Cutting method">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {[['laser', '🔥', 'Laser', '±0.1 mm — thin & precise'], ['plasma', '⚡', 'Plasma', '±0.5 mm — fast & thick']].map(([val, icon, name, desc]) => (
              <button key={val} onClick={() => set('method')(val)} style={{
                padding: '10px 12px', borderRadius: 8, border: `1px solid ${form.method === val ? '#22d3a5' : '#1e293b'}`,
                background: form.method === val ? 'rgba(34,211,165,0.08)' : '#0f1117', cursor: 'pointer', textAlign: 'left',
              }}>
        ...
        <Section label={`Quantity: ${form.quantity} part${form.quantity > 1 ? 's' : ''}`}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#475569', marginTop: 3 }}>
            <span>1</span><span>50+ → 15% off</span><span>100</span>
          </div>
        </Section>
        ...
        <div style={{ background: '#0f1117', borderRadius: 8, padding: '12px 14px', marginBottom: 14 }}>
          {calculating ? (
            ...
            <>
              {[['Material cost', `€${pricing.unit_material_cost}`], ['Cutting time', `€${pricing.unit_cutting_cost}`], ['Setup fee', `€${pricing.setup_fee}`]].map(([l, v]) => (
                <div key={l} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '4px 0', borderBottom: '1px solid #1e293b', color: '#94a3b8' }}>
                  <span>{l}</span><span style={{ color: '#e2e8f0' }}>{v}</span>
                </div>
              ))}
              {pricing.quantity_discount && (
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '4px 0', borderBottom: '1px solid #1e293b', color: '#22d3a5' }}>
                  <span>Quantity discount</span><span>−{pricing.quantity_discount}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, fontWeight: 600, padding: '10px 0 0', color: '#f8fafc' }}>
                <span>× {form.quantity} parts</span><span style={{ color: '#22d3a5' }}>€{pricing.total_price}</span>
              </div>
            </>
          ) : (
            <div style={{ fontSize: 13, color: '#475569', textAlign: 'center', padding: '8px 0' }}>Upload a file to see pricing</div>
          )}
        </div>

        <button onClick={saveQuote} disabled={!pricing || saving || !file || saved} style={{
          width: '100%', padding: '11px', borderRadius: 8, border: 'none', fontSize: 14, fontWeight: 600, cursor: pricing && !saved ? 'pointer' : 'not-allowed',
          background: saved ? '#14532d' : (pricing ? '#22d3a5' : '#1e293b'), color: saved ? '#4ade80' : (pricing ? '#0f1117' : '#475569'), transition: 'all 0.15s',
        }}>
...
function Chip({ active, onClick, children }) {
  return (
    <button onClick={onClick} style={{
      padding: '5px 12px', borderRadius: 99, border: `1px solid ${active ? '#22d3a5' : '#1e293b'}`,
      background: active ? 'rgba(34,211,165,0.12)' : '#0f1117', color: active ? '#22d3a5' : '#64748b',
